...
def generate_reverse_sorted_data(size):
    data = sorted([random.randint(0, 1000000) for _ in range(size)], reverse=True)
...
def generate_identical_data(size):
    data = [random.randint(0, 1000000) for _ in range(size)]
    identical_number = random.randint(0, 1000000)
...
def run_and_print(algorithm, data_filename, size):
    ...
    print(f"{size}, {comparisons}, {execution_time:.2f}")
